...
class HelsinkiNearYouResultsController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function content(Request $request) : array|RedirectResponse {
    ...
    if (!$address) {
      \Drupal::messenger()->addMessage($this->t('Please enter an address', [], ['context' => 'Helsinki near you']), 'error');
    ...
    if (!$addressData) {
      \Drupal::messenger()->addMessage($this->t('Address not found', [], ['context' => 'Helsinki near you']), 'error');
      return $this->redirect('helfi_etusivu.helsinki_near_you');
    }
