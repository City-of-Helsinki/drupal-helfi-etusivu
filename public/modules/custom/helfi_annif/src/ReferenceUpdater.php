...
final class ReferenceUpdater {
   ...
   */
  private function getAllReferenceFields(): array {
  ...
  /**
   * Adds missing annif entities to reference fields.
   *
  ...
  public function updateEntityReferenceFields(FieldableEntityInterface $entity): void {
    ...
    foreach ($fieldNames as $fieldName) {
      ...
      if ($referenceField->isEmpty()) {
        ...
        $this->copyLegacyData($entity, $topicsEntity);
  ...
  /**
   * Copy values from legacy field to the new entity.
   *
   * @todo remove this once the legacy field is deleted.
   */
  private function copyLegacyData(FieldableEntityInterface $entity, SuggestedTopicsInterface $topics): void {
    if (!$entity->hasField('annif_keywords')) {
      return;
    }

    $keywords = $entity->get('annif_keywords')->getValue();
    $count = count($keywords);
    $keywords = array_map(fn (array $item, int $index) => $item + [
      // The legacy data does not have scores. However, the keywords
      // are sorted by score, so this maps the index to range [0, 0.5].
      'score' => ($count - $index) / $count / 2,
    ], $keywords, array_keys($keywords));

    $topics->set('keywords', $keywords);
  }
